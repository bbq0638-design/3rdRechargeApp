...
export default function ReportAllScreen({
  user,
  filterType,
  allReports,
  onRefresh,
  ...
  const handlePressDetail = report => {
    ...
    if (type.includes('movie')) {
      ...
      const isPost = type.includes('post'); // moviepost, moviepostcomment
      const isComment = type.endsWith('comment'); // 댓글 여부

      params = {
        // post면 realId, popular 댓글이면 report.postId 사용
        movieId: isPost ? realId : report.postId,
        type: isPost ? 'post' : 'popular',
      };
